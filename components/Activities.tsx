          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {[...activities].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(act => {
                ...
                return (
                  <tr key={act.id} className="hover:bg-slate-50 dark:hover:bg-white/5 group">
                    <td className="px-6 py-4 text-slate-500 dark:text-dark-text-secondary">{act.date}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => setActivityToDelete(act.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
